...
[V1.0.0 一期需求总览](requirements/V1.0.0-一期需求总览.md) 已形成专业建议稿，包含8项独立1.0.0需求、74条业务规则及验收场景；需求待确认，同版本详设均为骨架。V1.0.0 是交付批次，不代替单项需求版本；其余历史候选见 [建设顺序](architecture/建设顺序.md)。

已确认范围：统一 PStock、沪深主板交易、外围研究、前后端分离、业务模块优先、需求独立版本、Claude Code 共用约束。前端技术框架已确认，见 [前端架构](architecture/前端架构.md)。[后端架构](architecture/后端架构.md) 已落实 Python/FastAPI、PostgreSQL、数据库持久任务及后续实时性演进；初期部署在持续在线的 4 核 4G 云服务器，优先消息面、技术面与公司研究。公司资料和研究入口覆盖全A（含创业板、科创板、北交所），不以自选数量限制资料范围，交易权限仍为本人已开通的沪深主板。交易模式、资金、周期、回撤、数据预算与精确依赖版本仍待确认。
